...
(async () => {
  ...
  try {
    ...
    if (!body) {
      warning(
        `👉 [github-action-modify-file-content]: "body" input value does not exist.`,
      );
      return;
    }
    if (!filepath) {
      warning(
        `👉 [github-action-modify-file-content]: "path" input value does not exist.`,
      );
      return;
    }
    if (REGEXP.test(body)) {
