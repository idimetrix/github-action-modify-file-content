...
export type FilePutQuery =
  paths["/repos/{owner}/{repo}/contents/{path}"]["put"]["requestBody"]["content"]["application/json"] &
    paths["/repos/{owner}/{repo}/contents/{path}"]["put"]["parameters"]["path"];
...
async function getFileContents(
  branch: string,
): Promise<GetContentResponseType | undefined> {
  ...
  try {
    ...
    warning(
      `👉 [github-action-modify-file-content]: Get File Contents: ${err instanceof Error ? err.message : err}`,
    );
    return;
  }
}
...
function getBodyContent(oldFileContent: string, content: string) {
  ...
  return oldFileContent.replace(
    REG,
    `${openDelimiter}${content}${closeDelimiter}`,
  );
}

export async function modifyPathContents(
  options: Partial<FilePutQuery> = {},
  content: string,
) {
  const { ...other } = options;
  const {
  ...
  if (!options.path) {
    throw new Error(
      `modifyPathContents: file directory parameter does not exist`,
    );
  }
  ...
  if (currentFile && "content" in currentFile) {
    ...
    if (oldFileContent == reuslt) {
      warning(
        `👉 [github-action-modify-file-content]: Content has not changed!!!!!`,
      );
      return;
    }
    body = {
    ...
    warning(
      `👉 [github-action-modify-file-content]: Not Found ::- ${options.path}`,
    );
    const result = await octokit.request(
      "PUT /repos/{owner}/{repo}/contents/{path}",
      {
